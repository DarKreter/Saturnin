...
using namespace std;
namespace{ /// Anonimowa przesten nazw tu tu ruuu
    short int licznik_zaliczonych=0; ///ZMIENNA GLOBALNA PARARARA
    int k_ty,k_przeciwnik,k_plansza;
}
...
void rysuj(string pole[8][8], short int y=11, short int x=11){
    /// y i x mowia ktora figure podswietlic
    saturnin_mowi(k_plansza);
    cout<<"#|";
    for(int i=0;i<=7;i++){
        cout<< nnl(i,0) << "|";
    }
    ...
    for(int i=7;i>=0;i--){
        ...
        cout<< i+1 <<"|";
        for(int j=0;j<=7;j++){
            saturnin_mowi(pole[i][j].substr(1,1)=="B" ? k_ty : pole[i][j].substr(1,1)=="C" ? k_przeciwnik : k_plansza);
            if(i == y && j == x) saturnin_mowi(5); /// Specjalny kolor
...
//        cout << "numer 1\n";
    ...
    /// Mam doœæ ide na przerwe SKONCZYLEM TWE DZIELO chyba
}
...
//    cout << "---------funkcja odkrywanie krola pod szach\n";
    ...
    string kolor_tury=pole[zaliczone[0]][zaliczone[1]].substr(1,1); ///ustawiamy za kolor tury kolor figury ktor¹ sie ruszamy
...
void Konwerter_planszy(string pole[8][8],int k_host, string host, string przeciwnik,int bicie_w_przelocie=11, bool czy_wykonany_przelot=false,bool czy_to_gosc=false,bool start=false){ /// Wsadza plansze do pliku
    fstream plik;
    plik.open(("../Szachy/Szachy-Serwer/"+host+"-"+przeciwnik+".txt").c_str(),ios::out);
    if(start==true){
        plik<<"start"<<endl;
    }

    if(czy_to_gosc==true){
        plik <<"ruch hosta"<<endl;
        plik << bicie_w_przelocie << endl << czy_wykonany_przelot << endl;
    }
    else{
        plik << "ruch goscia"<< endl << k_host << endl;
        plik << bicie_w_przelocie << endl << czy_wykonany_przelot << endl;
    }

    for(int i=7;i>=0;i--){
        for(int j=0;j<8;j++){
            if(pole[i][j].substr(1,1)=="B"){
                plik << pole[i][j].substr(0,1)+"C" << endl;
            }
            else if(pole[i][j].substr(1,1)=="C"){
                plik << pole[i][j].substr(0,1)+"B" << endl;
            }
            else plik << pole[i][j] << endl;
        }
    }
...
/// ***************************************************************************************************************************************
int gramy_gosc(string gosc,string host){
    fstream plik; string lokacja;
//    cout<<"tu"<<endl;
    plik.open("../Pliki/lokalizacja",ios::in);
    if(plik.good()== false){ cout<<"nie mozna odczytac pliku"<<endl; }
...
//    cout<<"tu"<<endl;
    /// TWORZYMY ZMIENNE
            string kolor_tury="B";
            bool szachowany_krol=0; /// szczerze nie wiem czy ma sie ustawiac to na 0
            ...
            bool mozliwosc_malej_roszady=true;
            bool mozliwosc_duzej_roszady=true;

            short int bicie_w_przelocie=11 ; /// numer pionka ktory ruszyl sie ostatnio o 2 pola ( to bedzie trzeba pobierac od przeciwnika!! )
            bool czy_wykonany_przelot=false; /// czy w ostatniej turze byl wykonany przelot ( to tez)
            int k_host,k_gosc,kolor_saturnina;
            bool czy_byl_juz_pierwszy_ruch=false;
    /// PETLA GRY
    for(int zmienna=0;;zmienna++){
            /// Resetowanie zmiennych
            licznik_pomocy=0; licznik_zaliczonych=0;


            /// POBIERANIE KOLORU GOSCIA
        plik.open((lokacja+"kolory").c_str(),ios::in);
        if(plik.good()== false) cout<<"nie mozna odczytac pliku"<<endl;
        plik >> kolor_saturnina >> k_gosc;
        plik.close();
//        cout<<host <<" " <<gosc<<endl;

        string napis;
            /// SPRAWDZANIE CZY JEST NASZ RUCH
        plik.open(("../Szachy/Szachy-Serwer/"+host+"-"+gosc+".txt").c_str(),ios::in);
        if(plik.good()==false){
            plik.close(); continue;
        }
        getline(plik,napis);
//        cout<<"napis- "<<napis<<endl;
        if(czy_byl_juz_pierwszy_ruch==false){
            if(napis != "start"){
                plik.close(); continue;
            }
            else {
                czy_byl_juz_pierwszy_ruch=true;
                getline(plik,napis);
            }
        }
        else{
            if(napis !="ruch goscia"){
            plik.close(); continue;
            }
        }


        /// JESLI JEST NASZ RUCH
            system("cls");
            /// POBRANIE KOLORU HOSTA
            plik >> k_host;
                ///***************************************************************************************************************************************************************************
                    /// NA POTRZEBY TESTOW
                    k_host=7;
//                    cout<<k_gosc<<endl;
            ...
            /// POBIERANIE NIEZBEDNYCH INFORMACJI
            plik >> bicie_w_przelocie >> czy_wykonany_przelot;
            /// USTAWIANIE ZMIENNYCH GLOBALNYCH
            k_ty=k_gosc;
            k_przeciwnik=k_host;
            k_plansza = 2;
            /// RYSOWANIE PLANSZY
            string miejsce;  /// szary moze potem zmienie na kolor saturnina
            string pole[8][8];
            kolor("plansza");
            for(int i=0;i<8;i++){
                for(int j=0;j<8;j++){
...
//            cout<<" PACIK KOCHANI"<<endl;
        ...
        /// WCZTUJEMY RUCH
        string pseudo_ruch, ruch;
        saturnin_mowi(kolor_saturnina);
        cout<< "ruch "<< gosc <<":";
        saturnin_mowi(k_gosc);
        cin >> pseudo_ruch;
...
//////                cout<<"BICIE W PRZELOCIE"<<bicie_w_przelocie<<endl;
                ...
                if(Sprawdzanie_ruchu_Krol( pomocniczy[i] , tablica , pole , &zaliczone[licznik_zaliczonych], kolor_tury , mozliwosc_malej_roszady, mozliwosc_duzej_roszady )==true){
                    mozliwosc_malej_roszady=false;
                    mozliwosc_duzej_roszady=false;
                }
...
//                        cout<<"ALERT KROL NIE ATAKOWANYYYYYYYYYYYYYYYYYYYYY"<<endl;
        ...
        short int wybor_pomocniczy[2]={0};
...
//                cout<<"prrrr"<<endl;
                if(Sprawdzanie_odkrywania_Krola_pod_szach(kupa,zaliczone[i],pole,atakujacy) == false)
                {
                        ...
                        cout<<"gowno-"<<zaliczonev2[j][0]<<zaliczonev2[j][1] <<endl;
                        getch();
                        j++;
                }
            }
...
//            getch();
        ...
        else if(zaliczone[0][0] == 6 && zaliczone[0][1] == 7 && ruch.substr(0,1) == "W")
        {
            ...
            /// WŁAŚCIWE WYKONANIE RUCHU
            //cout<<"to wszystko nie ma sensu"<<endl;
            string kolor_tury = "B"; /// Tak tylko dodaje jakby jakis debil zapomnial (bo zmienialo sie na a)
            cout<<" co jest-"<<zaliczone[0][0]<<zaliczone[0][1] <<endl;
        pole[zaliczone[0][0]][zaliczone[0][1]]="__";
       //cout<<" 1. " << kolor_tury<<endl;
        pole[lnn(ruch.substr(1,1))][lnn(ruch.substr(2,1))]=ruch.substr(0,1)+ kolor_tury;
        cout<<" kuku "<<zaliczone[0][0]<<":"<<zaliczone[0][1]<<":"<<pole[zaliczone[0][0]][zaliczone[0][1]]<<endl;
        //cout<<" 3. " <<pole[lnn(ruch.substr(1,1))][lnn(ruch.substr(2,1))] <<endl;
...
//        getch();
        ...
        ///PRZESYLAMY RUCH PRZECIWNIKOWI
        cout<<" kuku "<<zaliczone[0][0]<<":"<<zaliczone[0][1]<<":"<<pole[zaliczone[0][0]][zaliczone[0][1]]<<endl;
        Konwerter_planszy(pole,k_host,host,gosc,11,false,true);
...
int gramy_host(string host, string gosc ,int k_host, int k_gosc){
    fstream plik; string lokacja;
    system("cls");
    /// TWORZENIE PLANSZY
...
//    cout<<"tu"<<endl;
    ...
    plik.close();
//    cout<<"tu"<<endl;
    /// TWORZE SERWER GRY ( czyli tworze plik w ktorym bedziemy przesylac informacje )

    Konwerter_planszy(pole,k_host,host,gosc,11,0,0,1);

    /// POBIERANIE KOLORU HOSTA
        plik.open((lokacja+"kolory").c_str(),ios::in);
        if(plik.good()== false) cout<<"nie mozna odczytac pliku"<<endl;
        plik >> kolor_saturnina >> k_host;
        plik.close();
    ///***************************************************************************************************************************************************************************
                    /// NA POTRZEBY TESTOW
                    k_host=7;
//                    cout<<k_gosc<<endl;
    ...
    /// Rysuje plansze
        ...
        rysuj(pole);
        saturnin_mowi(kolor_saturnina); cout<<"Ruch " << gosc<<"..."<<endl;
    /// PETLA GRY
    for(int tura=0;;tura++){
    /// Resetowanie zmiennych
        licznik_pomocy=0; licznik_zaliczonych=0;
...
//                    cout<<k_gosc<<endl;
            ...
            ///WYKONYWANIE RUCHU PRZECIWNIKA / RYSOWANIE PLANSZY
            system ("cls");
            plik >> bicie_w_przelocie >> czy_wykonany_przelot;
            string miejsce;  /// szary moze potem zmienie na kolor saturnina
            for(int i=0;i<8;i++){
...
//        getch();
        ...
        ///PRZESYLAMY RUCH PRZECIWNIKOWI
        Konwerter_planszy(pole,k_host,host,gosc,11,false,false);
